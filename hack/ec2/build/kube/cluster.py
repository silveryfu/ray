...
def new_spec(name=None, create_bucket=True):
    ...
    print(get_state_bucket())

    if create_bucket:
        s3.create_bucket(get_state_bucket())
...
def create_cluster(ins_groups: dict = None):
    if ins_groups is not None:
        pass
...
@change_dir
def create_and_start(ins_groups: dict = None):
    create_cluster(ins_groups)
    start_cluster()
    # default addons
    # start_dashboard()
    # start_metric_server()
...
def main():
    ...
    cmds = {
        # cluster operations
        "up": lambda: create_and_start({"c4.xlarge": 3}),
